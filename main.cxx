...
static int process_request(struct mg_connection *conn) {
	...
	if (conn->is_websocket) {
      ...
      rsb_ws_bridge::listenerPtr = rsb_ws_bridge::factory.createListener(rsb::Scope(rsb_ws_bridge::futureScope), rsb_ws_bridge::config);
      rsb_ws_bridge::listenerPtr->addHandler(rsb::HandlerPtr(new rsb::DataFunctionHandler<std::string> (&receiveRsbData)));
 		}
...
static int ev_handler(struct mg_connection *conn, enum mg_event ev) {
	...
	if (ev == MG_POLL && conn->is_websocket) {
	  ...
	  if (rsb_ws_bridge::newRsbData) {
	    std::cout << "Send WS event with " << rsb_ws_bridge::newRsbPayload.size() << " bytes"<< std::endl;
      std::cout << "Payload: " << rsb_ws_bridge::newRsbPayload << std::endl;
	    mg_websocket_write(conn, 2, (const char*) rsb_ws_bridge::newRsbPayload.c_str(), rsb_ws_bridge::newRsbPayload.size());
...
int main(int argc, char **argv) {
  ...
  {
    std::list< std::pair<rsb::converter::ConverterPredicatePtr, typename rsb::converter::Converter<std::string>::Ptr> > converters;
    // Use a converter which can handle everything.
    converters.push_back(std::make_pair(rsb::converter::ConverterPredicatePtr(new rsb::converter::AlwaysApplicable()),
                                  typename rsb::converter::Converter<std::string>::Ptr(new rsb::converter::ByteArrayConverter())));
    rsb::converter::ConverterSelectionStrategy<std::string>::Ptr css =
        rsb::converter::ConverterSelectionStrategy<std::string>::Ptr(new rsb::converter::PredicateConverterList<std::string>(converters.begin(), converters.end()));

    // Configure a Listener object.
    rsb_ws_bridge::config = rsb_ws_bridge::factory.getDefaultParticipantConfig();

    std::set<rsb::ParticipantConfig::Transport> transports = rsb_ws_bridge::config.getTransports();
    for (std::set<rsb::ParticipantConfig::Transport>::const_iterator it =
        transports.begin(); it != transports.end(); ++it) {
    rsb::ParticipantConfig::Transport& transport = rsb_ws_bridge::config.mutableTransport(
        it->getName());
  ...
  mg_set_option(server, "listening_port", "8080");
	...
	std::cout << "Started on port " << mg_get_option(server, "listening_port") << std::endl;
    ...
    rsb_ws_bridge::currentTimer = rsb_ws_bridge::tv.tv_sec * 1000 + rsb_ws_bridge::tv.tv_usec / 1000;
    if(rsb_ws_bridge::currentTimer - rsb_ws_bridge::lastTimerCam >= 1000 /*ms*/){
      std::cout << "PPS: " << counter  << " polls / " << rsb_ws_bridge::currentTimer - rsb_ws_bridge::lastTimerCam << " ms" << std::endl;
